...
class HeterogeneousSchedulingProblem(ElementwiseProblem):
    ...
    def __init__(
        ...
        migration_levels = int(experiment_cfg["optimizer"].get("migration_ratio_levels", defer_levels))
        ...
        # 染色体由四段组成：CPU 开机数、GPU 开机数、时间迁移比例、空间迁移比例。
        xl = np.concatenate(
            [
    ...
    def decode(self, x: np.ndarray) -> SchedulePlan:
        ...
        migration_levels = max(1, int(self.experiment_cfg["optimizer"].get("migration_ratio_levels", defer_levels)))
        cpu = np.rint(x[:hours]).astype(int)
        gpu = np.rint(x[hours : 2 * hours]).astype(int)
        defer = np.rint(x[2 * hours : 3 * hours]).astype(int) / defer_levels
        migration = np.rint(x[3 * hours : 4 * hours]).astype(int) / migration_levels
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out["F"] = [
            ...
            metrics.load_imbalance + metrics.delay_sensitive_violation_rate * 2.0 + metrics.remote_task_count * 1e-4,
        ]
        out["G"] = [
...
def run_nsga2(
        ...
        rows.append(
            {
                ...
                "migration_ratio": ",".join(f"{value:.1f}" for value in schedule.migration_ratio.tolist()),
                **metrics.to_dict(),
            }
        )
    ...
    best_schedule = SchedulePlan(
        ...
        migration_ratio=np.array([float(value) for value in str(recommended["migration_ratio"]).split(",")], dtype=float),
    )
